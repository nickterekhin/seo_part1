...
void TestHitcount() {
  ...
  const vector<string> queries = {"the", "wall", "all", "is", "the is"};
    //const vector<string> queries = {"is"};
  const vector<string> expected = {
    Join(' ', vector{
      "the:",
      "{docid: 0, hitcount: 2}",
...
string document_generator(size_t num_of_lines, size_t num_of_words_per_line) {
    ifstream is;
    is.open("dict.txt");
    ...
    is.close();
    cout << "Generator keywords size: " << key_words.size() << endl;


    std::mt19937 rng;
    rng.seed(std::random_device()());
    std::uniform_int_distribution<std::mt19937::result_type> dist6(1, key_words.size() - 1);

    stringstream ss;
    for (int i = 0; i < num_of_lines; i++) {
        for (int j = 0; j < num_of_words_per_line; j++) {
            int index = dist6(rng);
            ss << key_words[index] << " ";
        }
        ss << "\n";
...
void TestSearchServer(istream& document_input, istream& query_input,
        ...
        SearchServer srv;
    {LOG_DURATION("Cretae InvertIndex")
        srv.UpdateDocumentBase(document_input);
    }
    {
        LOG_DURATION("Queries Stream")
        srv.AddQueriesStream(query_input, search_results_output);
    }
}

void TestPerformance() {
    const int DOC_COUNT = 50000;
    const int WORDS_COUNT = 10;
    const int WORD_LENGTH = 10;
    const int QUERY_COUNT = 50'000;

    stringstream ss, qs;

    ss << document_generator(DOC_COUNT,WORDS_COUNT);
    qs << document_generator(DOC_COUNT, WORDS_COUNT);
